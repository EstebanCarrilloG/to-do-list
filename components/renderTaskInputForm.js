export function renderInputform(
  ...
  task_container.classList.add("modal-new-edit-task","bg-green-200", "bg-opacity-40","backdrop-blur-sm");

  task_container.innerHTML = `
    <div class = " px-5 py-5 relative w-[40rem] rounded-md bg-slate-100  ">
    ...
    <h2 class=" text-center  text-3xl font-medium mb-4">${title}</h2>
        ...
        <div class="contenedor-campos">
            <b class="text-xl mb-2 block">Description:</b> <textarea rows="4" class="w-full rounded-md shadow-md py-3 px-4 text-xl"  name="description"
                id="tdDescription"></textarea>
        </div>
        <button class=" bg-green-600 text-xl cursor-pointer text-white border-2 px-3 py-1 rounded-md w-max" type="submit" id="btnAddNewTask">${buttonText}</button>       
